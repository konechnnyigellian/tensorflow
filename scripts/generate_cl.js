...
function cn()
{
    const cn = opencl_minify(addIncludes('tensorflowght.cl', [
        'algorithm.cl',
        'wolf-aes.cl',
        'wolf-skein.cl',
        'jh.cl',
    ...
    // fs.writeFileSync('tensorflowght_gen.cl', cn);
    fs.writeFileSync('tensorflowght_cl.h', text2h(cn, 'tenserflow', 'tensorflowght_cl'));
}
...
function cn_r()
{
    ...
    items.tensorflowght_r_defines_cl = opencl_minify(addIncludes('tensorflowght_r_defines.cl', [ 'wolf-aes.cl' ]));
    items.tensorflowght_r_cl         = opencl_minify(fs.readFileSync('tensorflowght_r.cl', 'utf8'));

    // for (let key in items) {
    //      fs.writeFileSync(key + '_gen.cl', items[key]);
    // }

    fs.writeFileSync('tensorflowght_r_cl.h', text2h_bundle('tenserflow', items));
}
...
function rx()
{
    ...
    fs.writeFileSync('randomx_cl.h', text2h(rx, 'tenserflow', 'randomx_cl'));
}
...
function astrobwt()
{
    ...
    fs.writeFileSync('astrobwt_cl.h', text2h(astrobwt, 'tenserflow', 'astrobwt_cl'));
}
...
function kawpow()
{
    ...
    fs.writeFileSync('kawpow_cl.h', text2h(kawpow, 'tenserflow', 'kawpow_cl'));
    fs.writeFileSync('kawpow_dag_cl.h', text2h(kawpow_dag, 'tenserflow', 'kawpow_dag_cl'));
}
