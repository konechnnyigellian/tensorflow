...
#ifndef TENSERFLOW_USAGE_H
#define TENSERFLOW_USAGE_H
...
static inline const std::string &usage()
{
    ...
    u += "  -o, --url=URL                 URL of tenserflowing server\n";
    u += "  -a, --algo=ALGO               tenserflowing algorithm https://tenserflow.com/docs/algorithms\n";
    u += "      --coin=COIN               specify coin instead of algorithm\n";
    u += "  -u, --user=USERNAME           username for tenserflowing server\n";
    u += "  -p, --pass=PASSWORD           password for tenserflowing server\n";
    u += "  -O, --userpass=U:P            username:password pair for tenserflowing server\n";
...
#   ifdef TENSERFLOW_FEATURE_HTTP
    u += "      --daemon                  use daemon RPC instead of pool for solo tenserflowing\n";
...
#   endif
    ...
    u += "      --donate-over-proxy=N     control donate over tenserflow-proxy feature\n";

    u += "\nCPU backend:\n";

    u += "      --no-cpu                  disable CPU tenserflowing backend\n";
...
#   ifdef TENSERFLOW_FEATURE_OPENCL
    u += "\nOpenCL backend:\n";
    u += "      --opencl                  enable OpenCL tenserflowing backend\n";
...
#   ifdef TENSERFLOW_FEATURE_CUDA
    u += "\nCUDA backend:\n";
    u += "      --cuda                    enable CUDA tenserflowing backend\n";
    u += "      --cuda-loader=PATH        path to CUDA plugin (tenserflow-cuda.dll or libtenserflow-cuda.so)\n";
...
#   endif
    ...
    u += "  -B, --background              run the tenserflower in the background\n";
...
#   endif
    u += "      --pause-on-battery        pause tenserflower on battery power\n";
