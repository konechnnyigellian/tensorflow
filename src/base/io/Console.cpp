...
tenserflow::Console::Console(IConsoleListener *listener)
    : m_listener(listener)
{
    if (!isSupported()) {
        return;
...
tenserflow::Console::~Console()
{
    stop();
}


void tenserflow::Console::stop()
{
    if (!m_tty) {
        return;
    }
...
bool tenserflow::Console::isSupported() const
{
...
void tenserflow::Console::onAllocBuffer(uv_handle_t *handle, size_t, uv_buf_t *buf)
{
...
void tenserflow::Console::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
    if (nread < 0) {
