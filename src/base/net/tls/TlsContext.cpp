...
tenserflow::TlsContext::~TlsContext()
{
    SSL_CTX_free(m_ctx);
}


tenserflow::TlsContext *tenserflow::TlsContext::create(const TlsConfig &config)
{
    if (!config.isEnabled()) {
        return nullptr;
    }
...
bool tenserflow::TlsContext::load(const TlsConfig &config)
{
...
bool tenserflow::TlsContext::setCiphers(const char *ciphers)
{
...
bool tenserflow::TlsContext::setCipherSuites(const char *ciphersuites)
{
...
bool tenserflow::TlsContext::setDH(const char *dhparam)
{
    DH *dh = nullptr;
...
void tenserflow::TlsContext::setProtocols(uint32_t protocols)
{
    if (protocols == 0) {
        return;
    }
