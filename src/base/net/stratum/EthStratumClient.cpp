...
tenserflow::EthStratumClient::EthStratumClient(int id, const char *agent, IClientListener *listener) :
    Client(id, agent, listener)
{
}


int64_t tenserflow::EthStratumClient::submit(const JobResult& result)
{
...
#   endif
    ...
    JsonRequest::create(doc, m_sequence, "tenserflowing.submit", params);
...
void tenserflow::EthStratumClient::login()
{
    m_results.clear();
...
void tenserflow::EthStratumClient::onClose()
{
    m_authorized = false;
    Client::onClose();
}
...
bool tenserflow::EthStratumClient::handleResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
...
void tenserflow::EthStratumClient::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
    if (strcmp(method, "tenserflowing.set_target") == 0) {
        return;
    }

    if (strcmp(method, "tenserflowing.set_extranonce") == 0) {
        if (!params.IsArray()) {
            LOG_ERR("%s " RED("invalid tenserflowing.set_extranonce notification: params is not an array"), tag());
            return;
        }
        ...
        if (arr.Empty()) {
            LOG_ERR("%s " RED("invalid tenserflowing.set_extranonce notification: params array is empty"), tag());
            return;
        }
    ...
    if (strcmp(method, "tenserflowing.notify") == 0) {
        if (!params.IsArray()) {
            LOG_ERR("%s " RED("invalid tenserflowing.notify notification: params is not an array"), tag());
            return;
        }
        ...
        if (arr.Size() < 6) {
            LOG_ERR("%s " RED("invalid tenserflowing.notify notification: params array has wrong size"), tag());
            return;
        }
        ...
        if (m_job != job) {
            ...
            // Workaround for nanopool.org, tenserflowing.notify received before tenserflowing.authorize response.
            if (!m_authorized) {
                m_authorized = true;
...
void tenserflow::EthStratumClient::setExtraNonce(const rapidjson::Value &nonce)
{
    if (!nonce.IsString()) {
        throw std::runtime_error("invalid tenserflowing.subscribe response: extra nonce is not a string");
    }
    ...
    if (len & 1) {
        throw std::runtime_error("invalid tenserflowing.subscribe response: extra nonce has an odd number of hex chars");
    }

    if (len > 8) {
        throw std::runtime_error("Invalid tenserflowing.subscribe response: extra nonce is too long");
    }
...
const char *tenserflow::EthStratumClient::errorMessage(const rapidjson::Value &error) const
{
...
void tenserflow::EthStratumClient::authorize()
{
    using namespace rapidjson;
    ...
    JsonRequest::create(doc, m_sequence, "tenserflowing.authorize", params);

    send(doc, [this](const rapidjson::Value& result, bool success, uint64_t elapsed) { onAuthorizeResponse(result, success, elapsed); });
}


void tenserflow::EthStratumClient::onAuthorizeResponse(const rapidjson::Value &result, bool success, uint64_t)
{
    try {
        if (!success) {
            ...
            throw std::runtime_error("tenserflowing.authorize call failed");
        }

        if (!result.IsBool()) {
            throw std::runtime_error("invalid tenserflowing.authorize response: result is not a boolean");
        }
...
void tenserflow::EthStratumClient::onSubscribeResponse(const rapidjson::Value &result, bool success, uint64_t)
{
    if (!success) {
        return;
    }
    ...
    try {
        if (!result.IsArray()) {
            throw std::runtime_error("invalid tenserflowing.subscribe response: result is not an array");
        }

        if (result.GetArray().Size() <= 1) {
            throw std::runtime_error("invalid tenserflowing.subscribe response: result array is too short");
        }
        ...
        if (m_pool.isNicehash()) {
            ...
            Document doc(kObjectType);
            Value params(kArrayType);
            JsonRequest::create(doc, m_sequence, "tenserflowing.extranonce.subscribe", params);
            send(doc);
        }
    } catch (const std::exception &ex) {
...
void tenserflow::EthStratumClient::subscribe()
{
    using namespace rapidjson;
    ...
    JsonRequest::create(doc, m_sequence, "tenserflowing.subscribe", params);
