...
tenserflow::NetworkState::NetworkState(IStrategyListener *listener) : StrategyProxy(listener)
{
}


#ifdef TENSERFLOW_FEATURE_API
rapidjson::Value tenserflow::NetworkState::getConnection(rapidjson::Document &doc, int version) const
{
...
rapidjson::Value tenserflow::NetworkState::getResults(rapidjson::Document &doc, int version) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void tenserflow::NetworkState::printConnection() const
{
    if (!m_active) {
...
void tenserflow::NetworkState::printResults() const
{
    if (!m_hashes) {
...
const char *tenserflow::NetworkState::scaleDiff(uint64_t &diff)
{
...
std::string tenserflow::NetworkState::humanDiff(uint64_t diff)
{
...
void tenserflow::NetworkState::onActive(IStrategy *strategy, IClient *client)
{
...
void tenserflow::NetworkState::onJob(IStrategy *strategy, IClient *client, const Job &job, const rapidjson::Value &params)
{
...
void tenserflow::NetworkState::onPause(IStrategy *strategy)
{
...
void tenserflow::NetworkState::onResultAccepted(IStrategy *strategy, IClient *client, const SubmitResult &result, const char *error)
{
    add(result, error);
...
uint32_t tenserflow::NetworkState::latency() const
{
...
uint64_t tenserflow::NetworkState::avgTime() const
{
    if (m_latency.empty()) {
        return 0;
    }
...
uint64_t tenserflow::NetworkState::connectionTime() const
{
    return m_active ? ((Chrono::steadyMSecs() - m_connectionTime)) : 0;
}


void tenserflow::NetworkState::add(const SubmitResult &result, const char *error)
{
    if (error) {
        m_rejected++;
        return;
...
void tenserflow::NetworkState::stop()
{
    m_active      = false;
    m_diff        = 0;
    m_ip          = nullptr;
