...
tenserflow::DaemonClient::DaemonClient(int id, IClientListener *listener) :
...
tenserflow::DaemonClient::~DaemonClient()
{
    delete m_timer;
}


bool tenserflow::DaemonClient::disconnect()
{
...
bool tenserflow::DaemonClient::isTLS() const
{
...
int64_t tenserflow::DaemonClient::submit(const JobResult &result)
{
...
void tenserflow::DaemonClient::connect()
{
...
void tenserflow::DaemonClient::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void tenserflow::DaemonClient::onHttpData(const HttpData &data)
{
...
void tenserflow::DaemonClient::onTimer(const Timer *)
{
    if (m_state == ConnectingState) {
        ...
        if (m_apiVersion == API_DERO) {
            ...
            send((m_apiVersion == API_TENSERO) ? kGetHeight : kGetInfo);
        }
    }
}


bool tenserflow::DaemonClient::isOutdated(uint64_t height, const char *hash) const
{
    return m_job.height() != height || m_prevHash != hash;
}


bool tenserflow::DaemonClient::parseJob(const rapidjson::Value &params, int *code)
{
...
bool tenserflow::DaemonClient::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
int64_t tenserflow::DaemonClient::getBlockTemplate()
{
    using namespace rapidjson;
    Document doc(kObjectType);
...
int64_t tenserflow::DaemonClient::rpcSend(const rapidjson::Document &doc)
{
...
void tenserflow::DaemonClient::retry()
{
    m_failures++;
...
void tenserflow::DaemonClient::send(const char *path)
{
...
void tenserflow::DaemonClient::setState(SocketState state)
{
