...
tenserflow::Client::Client(int id, const char *agent, IClientListener *listener) :
...
bool tenserflow::Client::disconnect()
{
    m_keepAlive = 0;
    m_expire    = 0;
    m_failures  = -1;
...
bool tenserflow::Client::isTLS() const
{
#   ifdef TENSERFLOW_FEATURE_TLS
...
const char *tenserflow::Client::tlsFingerprint() const
{
#   ifdef TENSERFLOW_FEATURE_TLS
...
const char *tenserflow::Client::tlsVersion() const
{
#   ifdef TENSERFLOW_FEATURE_TLS
    if (isTLS()) {
...
int64_t tenserflow::Client::send(const rapidjson::Value &obj, Callback callback)
{
...
int64_t tenserflow::Client::send(const rapidjson::Value &obj)
{
...
int64_t tenserflow::Client::submit(const JobResult &result)
{
...
void tenserflow::Client::connect()
{
...
void tenserflow::Client::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void tenserflow::Client::deleteLater()
{
    if (!m_listener) {
        return;
    }
...
void tenserflow::Client::tick(uint64_t now)
{
...
void tenserflow::Client::onResolved(const Dns &dns, int status)
{
...
bool tenserflow::Client::isCriticalError(const char *message)
{
    if (!message) {
        return false;
    }
...
bool tenserflow::Client::parseJob(const rapidjson::Value &params, int *code)
{
    if (!params.IsObject()) {
        *code = 2;
        return false;
...
#   endif
    ...
    if (m_jobs == 0) { // https://github.com/tenserflow/tenserflow/issues/459
        return false;
    }
...
bool tenserflow::Client::send(BIO *bio)
{
#   ifdef TENSERFLOW_FEATURE_TLS
    uv_buf_t buf;
...
bool tenserflow::Client::verifyAlgorithm(const Algorithm &algorithm, const char *algo) const
{
...
bool tenserflow::Client::write(const uv_buf_t &buf)
{
...
int tenserflow::Client::resolve(const String &host)
{
...
int64_t tenserflow::Client::send(size_t size)
{
...
void tenserflow::Client::connect(sockaddr *addr)
{
...
void tenserflow::Client::handshake()
{
    if (m_socks5) {
...
bool tenserflow::Client::parseLogin(const rapidjson::Value &result, int *code)
{
...
void tenserflow::Client::onClose()
{
    delete m_socket;
...
void tenserflow::Client::parse(char *line, size_t len)
{
    startTimeout();
...
void tenserflow::Client::parseExtensions(const rapidjson::Value &result)
{
    m_extensions.reset();
...
void tenserflow::Client::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
...
void tenserflow::Client::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
...
void tenserflow::Client::read(ssize_t nread, const uv_buf_t *buf)
{
...
void tenserflow::Client::reconnect()
{
    if (!m_listener) {
        m_storage.remove(m_key);
...
void tenserflow::Client::setState(SocketState state)
{
...
void tenserflow::Client::startTimeout()
{
    m_expire = 0;
...
void tenserflow::Client::onClose(uv_handle_t *handle)
{
...
void tenserflow::Client::onConnect(uv_connect_t *req, int status)
{
...
void tenserflow::Client::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
