...
tenserflow::Http::Http() :
    m_host(kLocalhost)
{
}


bool tenserflow::Http::isEqual(const Http &other) const
{
...
rapidjson::Value tenserflow::Http::toJSON(rapidjson::Document &doc) const
{
...
void tenserflow::Http::load(const rapidjson::Value &http)
{
    if (!http.IsObject()) {
        return;
    }
...
void tenserflow::Http::setPort(int port)
{
