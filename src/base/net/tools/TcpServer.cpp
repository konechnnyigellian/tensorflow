...
static const tenserflow::String kLocalHost("127.0.0.1");


tenserflow::TcpServer::TcpServer(const String &host, uint16_t port, ITcpServerListener *listener) :
...
tenserflow::TcpServer::~TcpServer()
{
    Handle::close(m_tcp);
}


int tenserflow::TcpServer::bind()
{
    if (!m_version) {
...
void tenserflow::TcpServer::create(uv_stream_t *stream, int status)
{
    if (status < 0) {
        return;
    }
...
void tenserflow::TcpServer::onConnection(uv_stream_t *stream, int status)
{
