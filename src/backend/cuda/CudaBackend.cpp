...
namespace tenserflow { static const char *kNvmlLabel = "NVML"; }
#endif
...
const char *tenserflow::cuda_tag()
{
    return Tags::nvidia();
}


tenserflow::CudaBackend::CudaBackend(Controller *controller) :
...
tenserflow::CudaBackend::~CudaBackend()
{
    delete d_ptr;
...
bool tenserflow::CudaBackend::isEnabled() const
{
    return d_ptr->controller->config()->cuda().isEnabled() && CudaLib::isInitialized() && !d_ptr->devices.empty();;
}


bool tenserflow::CudaBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cuda().threads().get(algorithm).isEmpty();
}


const tenserflow::Hashrate *tenserflow::CudaBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const tenserflow::String &tenserflow::CudaBackend::profileName() const
{
    return d_ptr->profileName;
}


const tenserflow::String &tenserflow::CudaBackend::type() const
{
    return kType;
}


void tenserflow::CudaBackend::execCommand(char)
{
}


void tenserflow::CudaBackend::prepare(const Job &job)
{
    if (d_ptr) {
...
void tenserflow::CudaBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void tenserflow::CudaBackend::printHealth()
{
#   ifdef TENSERFLOW_FEATURE_NVML
    d_ptr->printHealth();
#   endif
...
void tenserflow::CudaBackend::setJob(const Job &job)
{
    ...
    auto threads = cuda.get(d_ptr->controller->tenserflower(), job.algorithm(), d_ptr->devices);
...
void tenserflow::CudaBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void tenserflow::CudaBackend::stop()
{
...
void tenserflow::CudaBackend::tick(uint64_t ticks)
{
...
#ifdef TENSERFLOW_FEATURE_API
rapidjson::Value tenserflow::CudaBackend::toJSON(rapidjson::Document &doc) const
{
...
void tenserflow::CudaBackend::handleRequest(IApiRequest &)
{
}
#endif
