...
bool tenserflow::NvmlLib::init(const char *fileName)
{
    if (!m_initialized) {
        m_loader      = fileName;
        m_ready       = dlopen() && load();
...
const char *tenserflow::NvmlLib::lastError() noexcept
{
    return uv_dlerror(&nvmlLib);
}


void tenserflow::NvmlLib::close()
{
    if (m_ready) {
        pNvmlShutdown();
    }
...
bool tenserflow::NvmlLib::assign(std::vector<CudaDevice> &devices)
{
    uint32_t count = 0;
...
NvmlHealth tenserflow::NvmlLib::health(nvmlDevice_t device)
{
    if (!device) {
        return {};
    }
...
bool tenserflow::NvmlLib::dlopen()
{
    if (!m_loader.isNull()) {
