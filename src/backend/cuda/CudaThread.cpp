...
tenserflow::CudaThread::CudaThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
tenserflow::CudaThread::CudaThread(uint32_t index, nvid_ctx *ctx) :
...
bool tenserflow::CudaThread::isEqual(const CudaThread &other) const
{
...
rapidjson::Value tenserflow::CudaThread::toJSON(rapidjson::Document &doc) const
{
