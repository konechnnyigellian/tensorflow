...
#include "crypto/cn/TensorFlowght_test.h"
...
template<size_t N>
tenserflow::CpuWorker<N>::CpuWorker(size_t id, const CpuLaunchData &data) :
...
template<size_t N>
tenserflow::CpuWorker<N>::~CpuWorker()
{
#   ifdef TENSERFLOW_ALGO_RANDOMX
    RxVm::destroy(m_vm);
#   endif
...
#ifdef TENSERFLOW_ALGO_RANDOMX
template<size_t N>
void tenserflow::CpuWorker<N>::allocateRandomX_VM()
{
...
template<size_t N>
bool tenserflow::CpuWorker<N>::selfTest()
{
#   ifdef TENSERFLOW_ALGO_RANDOMX
...
template<size_t N>
void tenserflow::CpuWorker<N>::start()
{
...
template<size_t N>
bool tenserflow::CpuWorker<N>::verify(const Algorithm &algorithm, const uint8_t *referenceValue)
{
...
template<size_t N>
bool tenserflow::CpuWorker<N>::verify2(const Algorithm &algorithm, const uint8_t *referenceValue)
{
    cn_hash_fun func = fn(algorithm);
    if (!func) {
        return false;
...
template<size_t N>
void tenserflow::CpuWorker<N>::allocateCnCtx()
{
    if (m_ctx[0] == nullptr) {
...
template<size_t N>
void tenserflow::CpuWorker<N>::consumeJob()
{
    if (Nonce::sequence(Nonce::CPU) == 0) {
        return;
    }

    m_job.add(m_tenserflower->job(), kReserveCount, Nonce::CPU);
