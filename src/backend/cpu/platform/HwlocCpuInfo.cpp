...
tenserflow::HwlocCpuInfo::HwlocCpuInfo()
{
    m_threads = 0;
...
bool tenserflow::HwlocCpuInfo::membind(hwloc_const_bitmap_t nodeset)
{
    if (!hwloc_topology_get_support(m_topology)->membind->set_thisthread_membind) {
        return false;
    }
...
tenserflow::CpuThreads tenserflow::HwlocCpuInfo::threads(const Algorithm &algorithm, uint32_t limit) const
{
...
tenserflow::CpuThreads tenserflow::HwlocCpuInfo::allThreads(const Algorithm &algorithm, uint32_t limit) const
{
    CpuThreads threads;
    threads.reserve(m_threads);
...
void tenserflow::HwlocCpuInfo::processTopLevelCache(hwloc_obj_t cache, const Algorithm &algorithm, CpuThreads &threads, size_t limit) const
{
#   ifndef TENSERFLOW_ARM
