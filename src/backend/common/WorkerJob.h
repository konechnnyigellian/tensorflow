...
#ifndef TENSERFLOW_WORKERJOB_H
#define TENSERFLOW_WORKERJOB_H
...
template<>
inline uint32_t *tenserflow::WorkerJob<1>::nonce(size_t)
{
...
template<>
inline bool tenserflow::WorkerJob<1>::nextRound(uint32_t rounds, uint32_t roundSize)
{
    m_rounds[index()]++;
...
template<>
inline void tenserflow::WorkerJob<1>::save(const Job &job, uint32_t reserveCount, Nonce::Backend backend)
{
    m_index           = job.index();
    m_jobs[index()]   = job;
    m_rounds[index()] = 0;
