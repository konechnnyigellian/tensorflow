...
cl_program tenserflow::OclCache::build(const IOclRunner *runner)
{
...
#       ifdef _WIN32
        fileName = prefix() + "\\tenserflow\\.cache\\" + cacheKey(runner) + ".bin";
#       else
...
std::string tenserflow::OclCache::cacheKey(const char *deviceKey, const char *options, const char *source)
{
...
std::string tenserflow::OclCache::cacheKey(const IOclRunner *runner)
{
    return cacheKey(runner->deviceKey(), runner->buildOptions(), runner->source());
}


void tenserflow::OclCache::save(cl_program program, const std::string &fileName)
{
    size_t size = 0;
