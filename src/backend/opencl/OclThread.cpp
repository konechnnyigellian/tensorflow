...
tenserflow::OclThread::OclThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
bool tenserflow::OclThread::isEqual(const OclThread &other) const
{
...
rapidjson::Value tenserflow::OclThread::toJSON(rapidjson::Document &doc) const
{
