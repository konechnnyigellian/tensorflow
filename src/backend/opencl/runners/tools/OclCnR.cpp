...
#include "backend/opencl/cl/cn/tensorflowght_r_cl.h"
...
private:
    ...
    {
        std::string source(tensorflowght_r_defines_cl);

        for (size_t i = 0; i < OclCnR::kHeightChunkSize; ++i) {
            V4_Instruction code[256];
            const int code_size      = v4_random_math_init<Algorithm::CN_R>(code, offset + i);
            const std::string kernel = std::regex_replace(std::string(tensorflowght_r_cl), std::regex("TENSERFLOW_INCLUDE_RANDOM_MATH"), getCode(code, code_size));
...
cl_program tenserflow::OclCnR::get(const IOclRunner &runner, uint64_t height)
{
