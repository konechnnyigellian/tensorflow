...
cl_mem tenserflow::OclSharedData::createBuffer(cl_context context, size_t size, size_t &offset, size_t limit)
{
...
uint64_t tenserflow::OclSharedData::adjustDelay(size_t id)
{
    if (m_threads < 2) {
        return 0;
    }
...
#   ifdef TENSERFLOW_INTERLEAVE_DEBUG
...
uint64_t tenserflow::OclSharedData::resumeDelay(size_t id)
{
    if (m_threads < 2) {
        return 0;
    }
...
void tenserflow::OclSharedData::release()
{
...
void tenserflow::OclSharedData::setResumeCounter(uint32_t value)
{
    if (m_threads < 2) {
        return;
    }
...
void tenserflow::OclSharedData::setRunTime(uint64_t time)
{
...
#ifdef TENSERFLOW_ALGO_RANDOMX
cl_mem tenserflow::OclSharedData::dataset() const
{
    if (!m_dataset) {
...
void tenserflow::OclSharedData::createDataset(cl_context ctx, const Job &job, bool host)
{
    if (m_dataset) {
        return;
    }
