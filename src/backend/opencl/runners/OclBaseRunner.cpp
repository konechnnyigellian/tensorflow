...
tenserflow::OclBaseRunner::OclBaseRunner(size_t id, const OclLaunchData &data) :
    m_ctx(data.ctx),
...
#   ifdef TENSERFLOW_STRICT_OPENCL_CACHE
    m_deviceKey += ":";
...
tenserflow::OclBaseRunner::~OclBaseRunner()
{
...
size_t tenserflow::OclBaseRunner::bufferSize() const
{
    return align(Job::kMaxBlobSize) + align(sizeof(cl_uint) * 0x100);
}


uint32_t tenserflow::OclBaseRunner::deviceIndex() const
{
    return data().thread.index();
}


void tenserflow::OclBaseRunner::build()
{
...
void tenserflow::OclBaseRunner::init()
{
...
cl_mem tenserflow::OclBaseRunner::createSubBuffer(cl_mem_flags flags, size_t size)
{
...
size_t tenserflow::OclBaseRunner::align(size_t size) const
{
    return VirtualMemory::align(size, m_align);
}


void tenserflow::OclBaseRunner::enqueueReadBuffer(cl_mem buffer, cl_bool blocking_read, size_t offset, size_t size, void *ptr)
{
...
void tenserflow::OclBaseRunner::enqueueWriteBuffer(cl_mem buffer, cl_bool blocking_write, size_t offset, size_t size, const void *ptr)
{
...
void tenserflow::OclBaseRunner::finalize(uint32_t *hashOutput)
{
