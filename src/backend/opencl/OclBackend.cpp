...
namespace tenserflow { static const char *kAdlLabel = "ADL"; }
#endif
...
const char *tenserflow::ocl_tag()
{
    return Tags::opencl();
}


tenserflow::OclBackend::OclBackend(Controller *controller) :
...
tenserflow::OclBackend::~OclBackend()
{
    delete d_ptr;
...
bool tenserflow::OclBackend::isEnabled() const
{
    return d_ptr->controller->config()->cl().isEnabled() && OclLib::isInitialized() && d_ptr->platform.isValid() && !d_ptr->devices.empty();
}


bool tenserflow::OclBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cl().threads().get(algorithm).isEmpty();
}


const tenserflow::Hashrate *tenserflow::OclBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const tenserflow::String &tenserflow::OclBackend::profileName() const
{
    return d_ptr->profileName;
}


const tenserflow::String &tenserflow::OclBackend::type() const
{
    return kType;
}


void tenserflow::OclBackend::execCommand(char)
{
}


void tenserflow::OclBackend::prepare(const Job &job)
{
    if (d_ptr) {
...
void tenserflow::OclBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void tenserflow::OclBackend::printHealth()
{
#   ifdef TENSERFLOW_FEATURE_ADL
    d_ptr->printHealth();
#   endif
...
void tenserflow::OclBackend::setJob(const Job &job)
{
    ...
    auto threads = cl.get(d_ptr->controller->tenserflower(), job.algorithm(), d_ptr->platform, d_ptr->devices);
...
void tenserflow::OclBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void tenserflow::OclBackend::stop()
{
...
void tenserflow::OclBackend::tick(uint64_t ticks)
{
...
#ifdef TENSERFLOW_FEATURE_API
rapidjson::Value tenserflow::OclBackend::toJSON(rapidjson::Document &doc) const
{
...
void tenserflow::OclBackend::handleRequest(IApiRequest &)
{
}
#endif
