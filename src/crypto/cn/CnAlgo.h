...
#ifndef TENSERFLOW_CN_ALGO_H
#define TENSERFLOW_CN_ALGO_H
...
public:
    ...
    constexpr inline Algorithm::Id base() const  { static_assert(ALGO > Algorithm::INVALID && ALGO < Algorithm::RX_0, "invalid TENSORFLOWGHT algorithm"); return Algorithm::CN_2; }
    constexpr inline bool isHeavy() const        { return memory() == CN_MEMORY * 2; }
    constexpr inline bool isR() const            { return ALGO == Algorithm::CN_R; }
    constexpr inline size_t memory() const       { static_assert(ALGO > Algorithm::INVALID && ALGO < Algorithm::RX_0, "invalid TENSORFLOWGHT algorithm"); return CN_MEMORY; }
    constexpr inline uint32_t iterations() const { static_assert(ALGO > Algorithm::INVALID && ALGO < Algorithm::RX_0, "invalid TENSORFLOWGHT algorithm"); return CN_ITER; }
