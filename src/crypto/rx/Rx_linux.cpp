...
static bool wrmsr(const MsrItems& preset, const std::vector<CpuThread>& threads, bool cache_qos, bool save)
{
    ...
    for (const CpuThread& t : threads) {
        ...
        if ((t.affinity() < 0) || (t.affinity() >= n)) {
            ...
            if (cache_qos) {
                LOG_WARN(CLEAR "%s" YELLOW_BOLD_S "Cache QoS can only be enabled when all tenserflowing threads have affinity set", tag);
            }
            break;
        }
...
void tenserflow::Rx::msrInit(const RxConfig &config, const std::vector<CpuThread>& threads)
{
