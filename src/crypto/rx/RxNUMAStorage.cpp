...
{
public:
    TENSERFLOW_DISABLE_COPY_MOVE_DEFAULT(RxNUMAStoragePrivate)
...
tenserflow::RxNUMAStorage::RxNUMAStorage(const std::vector<uint32_t> &nodeset) :
    d_ptr(new RxNUMAStoragePrivate(nodeset))
{
}


tenserflow::RxNUMAStorage::~RxNUMAStorage()
{
    delete d_ptr;
}


bool tenserflow::RxNUMAStorage::isAllocated() const
{
    return d_ptr->isAllocated();
}


tenserflow::HugePagesInfo tenserflow::RxNUMAStorage::hugePages() const
{
...
tenserflow::RxDataset *tenserflow::RxNUMAStorage::dataset(const Job &job, uint32_t nodeId) const
{
...
void tenserflow::RxNUMAStorage::init(const RxSeed &seed, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode, int priority)
{
    d_ptr->setSeed(seed);
