...
tenserflow::MsrItem::MsrItem(const rapidjson::Value &value)
{
    if (!value.IsString()) {
        return;
    }
...
rapidjson::Value tenserflow::MsrItem::toJSON(rapidjson::Document &doc) const
{
    return toString().toJSON(doc);
}


tenserflow::String tenserflow::MsrItem::toString() const
{
